...
namespace SmsRu
{
	...
	public class SmsRuProvider : ISmsProvider
	{
		...
		public String login = "";//ConfigurationManager.AppSettings["smsRuLogin"];                              // Логин для доступа к SMS.RU
		public String password = "";//ConfigurationManager.AppSettings["smsRuPassword"];                        // Пароль для доступа к SMS.RU
		public String apiId = "";//ConfigurationManager.AppSettings["smsRuApiID"];                              // Является вашим секретным кодом, который используется во внешних программах
		public String partnerId = "";//ConfigurationManager.AppSettings["partnerId"];                           // Если вы участвуете в партнерской программе, укажите этот параметр в запросе
		public String SmsRuEmail = "";//ConfigurationManager.AppSettings["smsRuApiID"] + "@sms.ru";             // Ваш уникальный адрес (для отправки СМС по email)
		public String email = "";//ConfigurationManager.AppSettings["email"];                                   // Ваш email адрес для отправки
		public String smtpLogin = "";//ConfigurationManager.AppSettings["smtpLogin"];                           // Логин для авторизации на SMTP-сервере.
		public String smtpPassword = "";//ConfigurationManager.AppSettings["smtpPassword"];                     // Пароль для авторизации на SMTP-сервере.
		public String smtpServer = "";//ConfigurationManager.AppSettings["smtpServer"];                         // SMTP-сервер.
		public Int32 smtpPort = 496;//Convert.ToInt32(ConfigurationManager.AppSettings["smtpPort"]);             // Порт для авторизации на SMTP-сервере.
		public Boolean smtpUseSSL = true;//Convert.ToBoolean(ConfigurationManager.AppSettings["smtpUseSSL"]);     // Флаг - использовать SSL.
		public Boolean translit = false;//Convert.ToBoolean(ConfigurationManager.AppSettings["translit"]);         // Переводит все русские символы в латинские.
		public Boolean test = true;//Convert.ToBoolean(ConfigurationManager.AppSettings["test"]);                 // Имитирует отправку сообщения для тестирования ваших программ на правильность обработки ответов сервера. При этом само сообщение не отправляется и баланс не расходуется.
		...
		/*String dir = "C:\\smslog\\";
        String log_sent = "C:\\smslog\\sent.txt";
        String log_status = "C:\\smslog\\status.txt";
        String log_cost = "C:\\smslog\\cost.txt";
        String log_balance = "C:\\smslog\\balance.txt";
        String log_limit = "C:\\smslog\\limit.txt";
        String log_senders = "C:\\smslog\\sender.txt";
        String log_auth = "C:\\smslog\\auth.txt";
        String log_error = "C:\\smslog\\error.txt";
        String log_stoplist = "C:\\smslog\\stoplist.txt";*/
		#endregion
		...
		public String Send(String from, String[] to, String text, DateTime dateTime, EnumAuthenticationTypes authType)
		{
			...
			using (AtLogWriter writer = new AtLogWriter())
			{
				...
				try
				{
					...
					{
						/*using (AtLogWriter w= new AtLogWriter())
                        {
